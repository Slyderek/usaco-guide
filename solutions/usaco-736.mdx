...
author: Mrinall Umasudhan, Ananth Kashyap, Ben Dodge, Dongkyu Choi
---
...
with open('cownomics.in') as read:
    ...
    spotty, plain = [], []
    for _ in range(N):
        spotty.append([c for c in read.readline()])
    for _ in range(N):
        plain.append([c for c in read.readline()])

# for each place, check if there's any common character
# between spotty set and plain set
valid_locations = 0
for k in range(M):
    spotty_set = set(spotty[n][k] for n in range(N))
    plain_set = set(plain[n][k] for n in range(N))

    # place is valid if spotty and plain are exclusive for the place
    if len(spotty_set.intersection(plain_set)) == 0:
        valid_locations += 1

print(valid_locations, file=open('cownomics.out', 'w'))
```
